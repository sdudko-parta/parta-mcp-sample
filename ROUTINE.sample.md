...
3. The skill exits silently if the per-course `.sync.json` shows no delta — that is the expected outcome on most days. Don't treat "no changes" as an error.
...
- `<dir>: synced <project URL> — + <created> pages, ~ <updated>, ↕ <reordered>, - <deleted>; assets + <uploaded>, ~ <re-uploaded>; state commit <sha>` if there was a delta.
